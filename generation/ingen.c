...
extern FILE *fout;
extern  A_TYPE *int_type, *float_type, *char_type, *void_type, *string_type; //extern
extern  A_LITERAL literal_table[]; // extern
...
void gen_expression(A_NODE *node)
{
	...
	switch(node->name) {
				...
				case ID_PARM:
					switch (t->kind) {
						...
						case T_UNION:
							gen_code_i(LDA, id->level, id->address);
							i=id-> type-> size;
							gen_code_i(LDI,0,1%4?i/4+1:1/4);
							break;
		...
		case N_EXP_FUNCTION_CALL :
			...
			gen_code_i(CAL,0,0);
			break;
		case N_EXP_STRUCT: // clear not defind 
...
void gen_expression_left(A_NODE *node)
{
	...
	switch(node->name) {
		...
		case N_EXP_ARRAY :
			gen_expression(node->llink);
			gen_expression(node->rlink);
// gen_code_i(CHK,0, node-> link-> type->expr);
			if (node-> type-> size>1) {
				gen_code_i(LITI,0,node-> type->size);
				gen_code_i(MULI,0,0);
			}
...
{ 
		...
		switch(node->name) {
				...
				case N_STMT_LABEL_DEFAULT : break;
				case N_STMT_COMPOUND:
				if(node->llink) gen_declaration(node->llink); 
				gen_statement_list(node->clink ,cnt_label, brk_label);
				break;
		...
		case N_STMT_IF_ELSE:
				...
				gen_label(l2,0);
				break;
			case N_STMT_SWITCH: break;
			case N_STMT_WHILE:
				l3=get_label();
				gen_label(l1=get_label(),0);
				gen_expression(node->llink);
				gen_code_l(JPC, 0, l2=get_label());
				gen_statement(node->clink,l3,l2);
				gen_label(l3,0);
				gen_code_l(JMP, 0, l1);
				gen_label(l2,0);
				break;
		...
		case N_STMT_FOR:
				n=node->llink;
				l3=get_label();
				if (n->llink) gen_expression(n->llink);
				gen_label(l1=get_label(),0);
				l2=get_label();
				if (n->clink) { gen_expression(n->clink); gen_code_l(JPC, 0, l2);}
				gen_statement(node->clink,l3,l2);
				gen_label(l3,0);
				if (n->rlink) gen_expression(n->rlink);
				gen_code_l(JMP, 0, l1);
				gen_label(l2,0);
				break;
...
void gen_declaration(A_ID *id)
{
	...
	switch (id-> kind) {
		case ID_FUNC:
			if (id-> type->expr)
			{
				gen_label(0,id->name);// function name output
