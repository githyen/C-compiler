...
A_ID *linkDeclaratorList(A_ID *id_a, A_ID *id_b)
{
		A_ID *m=id_a;
		if(id_a==NIL)
				return(id_b);
...
BOOLEAN isPointOrArrayType(A_TYPE *t)
{
		if(t!=NIL)
				return(TRUE);
		else
...
void syntax_error(int i,char *s) {
	...
	switch (i) { case 11: printf("illegal referencing struct or union identifier %s",s); break; 
