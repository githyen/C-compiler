...
extern void setTypeSize(A_TYPE *, int); 
...
{
		if(isPointOrArrayType(t1)||isPointOrArrayType(t2))
...
BOOLEAN isAllowableAssignmentConversion(A_TYPE *t1, A_TYPE *t2, A_NODE *node) // t1 <--- t2
{
	...
	if (isArithmeticType(t1) && isArithmeticType(t2))
		return (TRUE);
	else if(isStructOrUnionType(t2) && isCompatibleType(t1,t2))
		return (TRUE);
		...
		BOOLEAN lvalue=FALSE;
			...
			case N_EXP_STRUCT :
		 	...
		 	case N_EXP_CAST : 
				...
				if (!isAllowableCastingConversion(result,t)) // check 
...
int sem_statement_list(A_NODE *node, int addr, A_TYPE *ret, BOOLEAN sw, BOOLEAN brk, BOOLEAN cnt)
{
	...
	switch(node->name) { 
			case N_STMT_LIST:
				...
				i=sem_statement(node->rlink,addr,ret,sw,brk,cnt);
				if(size<i)
					size=i;
				break; 
...
int sem_declaration(A_ID *id, int addr)
{
	...
	case ID_TYPE:
		i=sem_A_TYPE(id->type);
		break;
	default: semantic_error(89,id->line,id->name);
		break;
	}
	return (result);
}
